...
class Options_Machine {
			...
			switch ( $value['type'] ) {
              ...
              'semi bold italic'=>'Semi-Bold Italic',
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'birdwp-theme').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		//initialize all vars
		$slidevars = array('title','url','link','description','margintop','aspeed');
    ...
    $slider .= '<label>Description Margin top (optional, px)</label>';
    $slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][margintop]" id="'. $id .'_'.$order .'_slide_margintop" value="'. $val['margintop'] .'" />';

    $slider .= '<label>Description Animation speed (optional, 100..3000 and more)</label>';
    $slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][aspeed]" id="'. $id .'_'.$order .'_slide_aspeed" value="'. $val['aspeed'] .'" />';
