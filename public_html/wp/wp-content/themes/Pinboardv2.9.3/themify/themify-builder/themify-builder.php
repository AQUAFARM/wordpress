...
if(!function_exists('themify_manage_builder_cache')){
	...
	function themify_manage_builder_cache($data=array()) {
		if( TFCache::check_version()){
			...
			$output = sprintf( '<p><label for="%s"><input type="radio" id="%s" name="%s" value="%s" %s> %s</label><label for="%s"><input type="radio" id="%s" name="%s" value="%s" %s> %s</label><br/>%s</p>',
					esc_attr( $pre . 'cache' ),
					esc_attr( $pre . 'cache' ),
					esc_attr( $pre . 'cache' ),
					esc_attr( 'disable' ),
					...
					esc_attr( 'Enable' ),
					__('Disable/Enable Builder caching (if you experience Builder issues and conflicts, disable it)', 'themify')
			);
